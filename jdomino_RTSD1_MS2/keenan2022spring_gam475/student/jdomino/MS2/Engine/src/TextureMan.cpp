...
Texture *TextureMan::Add(const char *const pName, Texture::Name name)
{
	...
	// Initialize the date
	pNode->Set(pName, name, textureID, GL_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE);
...
void TextureMan::IncrementTextureCount(Texture::Name name)
{
	TextureMan* pMan = TextureMan::privGetInstance();
	assert(pMan != nullptr);
	...
	pTexture->TextureCount++;

	Trace::out("texture Count: %d \n",pTexture->TextureCount);
}

void TextureMan::DecrementTextureCount(Texture::Name name)
{
	TextureMan* pMan = TextureMan::privGetInstance();
	assert(pMan != nullptr);
...
void TextureMan::DeleteTexture(Texture::Name name)
{

	TextureMan* pMan = TextureMan::privGetInstance();
	assert(pMan != nullptr);

	Texture* tmp = pMan->Find(name);

	DLink* textureNode = (DLink*)tmp;

	pMan->baseRemove(textureNode);
}

Texture* TextureMan::FindNext(Texture::Name _name)
{
	TextureMan* pMan = TextureMan::privGetInstance();
	assert(pMan != nullptr);

	Texture* tmp = pMan->Find(_name);


	if (tmp->pNext != nullptr)
	{
		tmp = (Texture*)tmp->pNext;
	}
	else if (tmp->pPrev != nullptr)
	{
		tmp = (Texture*)tmp->pPrev;
	}
	else
	{
		tmp = nullptr;
